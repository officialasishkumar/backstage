...
import { createCatalogModelFragment } from '@backstage/catalog-model/alpha';

describe('catalogModelRegistryServiceRef', () => {
  it('should serve registered kind fragments via the router', async () => {
    ...
    const res = await request(server).get(
      '/api/test/.backstage/catalog-model/v1/fragments',
    );
  ...
  it('should return empty fragments when nothing is registered', async () => {
    const testPlugin = createBackendPlugin({
      pluginId: 'test',
      register(env) {
        env.registerInit({
    ...
    expect(res.status).toEqual(200);
    expect(res.body).toEqual({ pluginId: 'test', fragments: [] });
  });
});
