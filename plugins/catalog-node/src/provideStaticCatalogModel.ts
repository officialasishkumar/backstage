...
/**
 * Creates a backend module that registers static catalog model fragments.
 *
 * @alpha
 * @remarks
 *
 * This is a convenience function for registering catalog model fragments
 ...
 *     fragments: [templateModelFragment],
 *   }),
 * );
 * ```
 */
export function provideStaticCatalogModel(options?: {
  fragments?: CatalogModelFragment[];
}) {
  return createBackendModule({
    ...
    register(reg) {
      reg.registerInit({
        ...
        async init({ model }) {
          model.addModelFragment(defaultCatalogEntityModel);
          for (const fragment of options?.fragments ?? []) {
            model.addModelFragment(fragment);
          }
        },
      });
    },
