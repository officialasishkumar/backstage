...
/**
 * Compiles a set of catalog model fragments into a single unified
 * catalog model.
 *
 * @alpha
 * @param inputs - The fragments to compile.
 * @returns The compiled catalog model.
 */
export function compileCatalogModel(
  inputs: Iterable<CatalogModelFragment>,
): CatalogModel {
  // Collect all ops from all inputs
  let allOps: CatalogModelOp[] = [];
  for (const input of inputs) {
    const internal = OpaqueCatalogModelFragment.toInternal(input);
